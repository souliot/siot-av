...
func TestParseNALUType(t *testing.T) {
	golden := map[uint8]uint8{
		0x06: NALUTypeSEI,
		0x41: NALUTypeSlice,
		0x65: NALUTypeIDRSlice,
	}
	for in, out := range golden {
		actual := ParseNALUType(in)
		assert.Equal(t, out, actual)

		b := ParseNALUTypeReadable(in)
		t.Log(b)
	}
}
...
func TestParseSliceType(t *testing.T) {
	...
	for _, item := range golden {
		v, err := ParseSliceType(item.in)
		assert.Equal(t, nil, err)
		assert.Equal(t, item.out, v)

		b, err := ParseSliceTypeReadable(item.in)
		assert.Equal(t, nil, err)
		t.Log(b)
	}
}
...
func TestSPSPPSSeqHeader2AnnexB(t *testing.T) {
	out, err := SPSPPSSeqHeader2AnnexB(goldenSeqHeader)
	assert.Equal(t, nil, err)
	var expected []byte
	expected = append(expected, NALUStartCode4...)
...
func TestParseSPSPPSFromSeqHeader(t *testing.T) {
	sps, pps, err := ParseSPSPPSFromSeqHeader(goldenSeqHeader)
	assert.Equal(t, nil, err)
...
func TestBuildSeqHeaderFromSPSPPS(t *testing.T) {
	sh, err := BuildSeqHeaderFromSPSPPS(goldenSPS, goldenPPS)
	assert.Equal(t, nil, err)
...
func TestParseSPS(t *testing.T) {
	...
	err = ParseSPS(nil, &ctx)
	assert.Equal(t, true, nazaerrors.Is(err, nazabits.ErrNazaBits))
	t.Logf("%+v", err)
}

func TestTry(t *testing.T) {
	err := TryParseSeqHeader(goldenSeqHeader)
	assert.Equal(t, nil, err)
}

func TestCorner(t *testing.T) {
	sps, pps, err := ParseSPSPPSFromSeqHeader([]byte{0})
	assert.Equal(t, nil, sps)
	assert.Equal(t, nil, pps)
	assert.Equal(t, ErrAVC, err)

	b := &bytes.Buffer{}
	err = CaptureAVCC2AnnexB(b, []byte{0x17, 0x0, 0x1})
