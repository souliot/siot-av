...
func NewBaseInSession(uniqueKey string, cmdSession IInterleavedPacketWriter, logger log.Logger) *BaseInSession {
	s := &BaseInSession{
		UniqueKey: uniqueKey,
		stat: base.StatSession{
	...
	s.log.Info("[%s] lifecycle new rtsp BaseInSession. session=%p", uniqueKey, s)
	return s
}
func (s *BaseInSession) Log() log.Logger {
	if s.log == nil {
...
func NewBaseInSessionWithObserver(uniqueKey string, cmdSession IInterleavedPacketWriter, observer BaseInSessionObserver, logger log.Logger) *BaseInSession {
	s := NewBaseInSession(uniqueKey, cmdSession, logger)
	s.observer = observer
	return s
}
...
func (session *BaseInSession) InitWithSDP(rawSDP []byte, sdpLogicCtx sdp.LogicContext) {
	...
	if session.sdpLogicCtx.IsAudioUnpackable() {
		...
		session.Log().Warn("[%s] audio unpacker not support for this type yet.", session.UniqueKey)
	}
	if session.sdpLogicCtx.IsVideoUnpackable() {
		...
		session.Log().Warn("[%s] video unpacker not support this type yet.", session.UniqueKey)
	}
...
func (session *BaseInSession) Dispose() error {
	session.Log().Info("[%s] lifecycle dispose rtsp BaseInSession. session=%p", session.UniqueKey, session)
...
func (session *BaseInSession) onReadRTPPacket(b []byte, rAddr *net.UDPAddr, err error) bool {
	if err != nil {
		session.Log().Error("[%s] read udp packet failed. err=%+v", session.UniqueKey, err)
		return true
	}
...
func (session *BaseInSession) handleRTCPPacket(b []byte, rAddr *net.UDPAddr) error {
	...
	if session.loggedReadRTCPCount < session.debugLogMaxCount {
		session.Log().Debug("[%s] LOGPACKET. read rtcp=%s", session.UniqueKey, hex.Dump(nazastring.SubSliceSafety(b, 32)))
	...
	case rtprtcp.RTCPPacketTypeSR:
		...
		if session.loggedReadSRCount < session.debugLogMaxCount {
			session.Log().Debug("[%s] LOGPACKET. %+v", session.UniqueKey, sr)
		...
		default:
			...
			//session.Log().Warn("[%s] read rtcp sr but senderSSRC invalid. senderSSRC=%d, audio=%d, video=%d",
			//	p.UniqueKey, sr.SenderSSRC, p.audioSSRC, p.videoSSRC)
			return ErrRTSP
		}
	default:
		session.Log().Warn("[%s] handleRTCPPacket but type unknown. type=%d", session.UniqueKey, b[1])
		return ErrRTSP
	}
...
func (session *BaseInSession) handleRTPPacket(b []byte) error {
	...
	if !session.sdpLogicCtx.IsPayloadTypeOrigin(packetType) {
		session.Log().Error("[%s] handleRTPPacket but type invalid. type=%d", session.UniqueKey, packetType)
		return ErrRTSP
	}

	h, err := rtprtcp.ParseRTPPacket(b)
	if err != nil {
		session.Log().Error("[%s] handleRTPPacket invalid rtp packet. err=%+v", session.UniqueKey, err)
		return err
	}
	...
	if session.sdpLogicCtx.IsAudioPayloadTypeOrigin(packetType) {
		if session.loggedReadAudioRTPCount < session.debugLogMaxCount {
			session.Log().Debug("[%s] LOGPACKET. read audio rtp=%+v", session.UniqueKey, h)
		...
		if session.loggedReadVideoRTPCount < session.debugLogMaxCount {
			session.Log().Debug("[%s] LOGPACKET. read video rtp=%+v", session.UniqueKey, h)
