...
func NewClientCommandSession(t ClientCommandSessionType, uniqueKey string, observer ClientCommandSessionObserver, logger log.Logger, modOptions ...ModClientCommandSessionOption) *ClientCommandSession {
	...
	s.log.WithPrefix("pkg.rtsp.client_command_session")
	s.log.Info("[%s] lifecycle new rtsp ClientCommandSession. session=%p", uniqueKey, s)
	return s
}
func (s *ClientCommandSession) Log() log.Logger {
	if s.log == nil {
...
func (session *ClientCommandSession) Dispose() error {
	session.Log().Info("[%s] lifecycle dispose rtsp ClientCommandSession. session=%p", session.UniqueKey, session)
	if session.conn == nil {
		return nil
	}
...
func (session *ClientCommandSession) runReadLoop() {
	...
	session.Log().Debug("[%s] start get_parameter timer.", session.UniqueKey)
	var r = bufio.NewReader(session.conn)
...
func (session *ClientCommandSession) connect(rawURL string) (err error) {
	...
	session.Log().Debug("[%s] > tcp connect.", session.UniqueKey)
	...
	session.Log().Debug("[%s] < tcp connect. laddr=%s, raddr=%s", session.UniqueKey, conn.LocalAddr().String(), conn.RemoteAddr().String())
...
func (session *ClientCommandSession) writeCmd(method, uri string, headers map[string]string, body string) error {
	...
	session.Log().Debug("[%s] > write %s.", session.UniqueKey, method)
	//session.Log().Debug("[%s] > write %s. req=%s", session.UniqueKey, method, req)
...
func (session *ClientCommandSession) writeCmdReadResp(method, uri string, headers map[string]string, body string) (ctx nazahttp.HTTPRespMsgCtx, err error) {
	for i := 0; i < 2; i++ {
		...
		session.Log().Debug("[%s] < read response. version=%s, code=%s, reason=%s, headers=%+v, body=%s",
