...
func NewPullSession(logger log.Logger, modOptions ...ModPullSessionOption) *PullSession {
	...
	s.log.WithPrefix("pkg.httpflv.pull_session")
	s.log.Info("[%s] lifecycle new httpflv PullSession. session=%p", uk, s)
	return s
}
...
func (session *PullSession) Dispose() {
	session.Log().Info("[%s] lifecycle dispose httpflv PullSession.", session.UniqueKey)
	if session.conn != nil {
...
func (session *PullSession) writeHTTPRequest() error {
	// # 发送 http GET 请求
	session.Log().Debug("[%s] > W http request. GET %s", session.UniqueKey, session.urlCtx.PathWithRawQuery)
...
func (session *PullSession) readHTTPRespHeader() (statusLine string, headers map[string]string, err error) {
	...
	session.Log().Debug("[%s] < R http response header. code=%s", session.UniqueKey, code)
	return
}
...
func (session *PullSession) readFLVHeader() ([]byte, error) {
	...
	session.Log().Debug("[%s] < R http flv header.", session.UniqueKey)
