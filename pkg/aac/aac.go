...
func NewADTS(logger log.Logger) ADTS {
	logger.WithPrefix("pkg.aac.acc")
	return ADTS{
		log: logger,
	}
}

func (a *ADTS) Log() log.Logger {
	if a.log == nil {
		a.log = log.DefaultBeeLogger
	}
	a.log.WithPrefix("pkg.aac.acc")
	return a.log
}
...
func (a *ADTS) InitWithAACAudioSpecificConfig(asc []byte) error {
	if len(asc) < minASCLength {
		a.Log().Warn("aac seq header length invalid. len=%d", len(asc))
		return ErrAAC
	}
...
func (a *ADTS) CalcADTSHeader(length uint16) ([]byte, error) {
	if !a.HasInited() {
		a.Log().Warn("calc adts header but asc not inited.")
		return nil, ErrAAC
	}
	// <ISO_IEC_14496-3.pdf>
...
func ParseAACSeqHeader(b []byte) (sh SequenceHeader, adts ADTS, err error) {
	if len(b) < 4 {
		err = fmt.Errorf("aac seq header length invalid. len=%d", len(b))
		return
	}
	adts = NewADTS(log.DefaultBeeLogger)
	...
	//	a.Log().Debug("%s %+v", hex.Dump(payload[:4]), sh)
