...
func (s *LogicContext) Log() log.Logger {
	if s.log == nil {
		s.log = log.DefaultBeeLogger
	}
	s.log.WithPrefix("pkg.sdp.logic")
	return s.log
}
...
func ParseSDP2LogicContext(b []byte) (LogicContext, error) {
	...
	ret.log = log.DefaultBeeLogger
	ret.log.WithPrefix("pkg.sdp.logic")
		...
		case "audio":
			...
			if md.ARTPMap.EncodingName == ARTPMapEncodingNameAAC {
				...
				if md.AFmtPBase != nil {
					...
					ret.Log().Warn("aac afmtp not exist.")
				}
			} else {
		...
		case "video":
			...
			case ARTPMapEncodingNameH264:
				...
				if md.AFmtPBase != nil {
					...
					ret.Log().Warn("avc afmtp not exist.")
				}
			case ARTPMapEncodingNameH265:
				...
				if md.AFmtPBase != nil {
					...
					ret.Log().Warn("hevc afmtp not exist.")
				}
			default:
