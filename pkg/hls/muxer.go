...
func NewMuxer(streamName string, config *MuxerConfig, observer MuxerObserver, logger log.Logger) *Muxer {
	...
	streamer := NewStreamer(m, logger)
	m.streamer = streamer
	m.log = logger
	m.log.WithPrefix("pkg.hls.muxer")
	m.log.Info("[%s] lifecycle new hls muxer. muxer=%p, streamName=%s", uk, m, streamName)
	return m
}

func (m *Muxer) Log() log.Logger {
	if m.log == nil {
		m.log = log.DefaultBeeLogger
	}
	m.log.WithPrefix("pkg.hls.muxer")
	return m.log
}
...
func (m *Muxer) Start() {
	m.Log().Info("[%s] start hls muxer.", m.UniqueKey)
	m.ensureDir()
}

func (m *Muxer) Dispose() {
	m.Log().Info("[%s] lifecycle dispose hls muxer.", m.UniqueKey)
	m.streamer.FlushAudio()
	if err := m.closeFragment(true); err != nil {
		m.Log().Error("[%s] close fragment error. err=%+v", m.UniqueKey, err)
	}
}
...
func (m *Muxer) OnFrame(streamer *Streamer, frame *mpegts.Frame) {
	...
	if frame.Sid == mpegts.StreamIDAudio {
		...
		if err := m.updateFragment(frame.PTS, boundary); err != nil {
			m.Log().Error("[%s] update fragment error. err=%+v", m.UniqueKey, err)
			return
		}

		if !m.opened {
			m.Log().Warn("[%s] OnFrame A not opened.", m.UniqueKey)
			return
		}
		...
		if !m.opened {
			m.Log().Warn("[%s] OnFrame V not opened.", m.UniqueKey)
			return
		}
	}
	...
	mpegts.PackTSPacket(frame, func(packet []byte) {
		if m.config.Enable {
			if err := m.fragment.WriteFile(packet); err != nil {
				m.Log().Error("[%s] fragment write error. err=%+v", m.UniqueKey, err)
				return
			}
		}
		if m.observer != nil {
...
func (m *Muxer) updateFragment(ts uint64, boundary bool) error {
	...
	if m.opened {
		...
		if (ts > m.fragTS && ts-m.fragTS > maxfraglen) || (m.fragTS > ts && m.fragTS-ts > negMaxfraglen) {
			m.Log().Warn("[%s] force fragment split. fragTS=%d, ts=%d", m.UniqueKey, m.fragTS, ts)
...
func (m *Muxer) writeRecordPlaylist(isLast bool) {
	...
	if err == nil {
		...
		if err != nil {
			m.Log().Error("[%s] update target duration failed. err=%+v", m.UniqueKey, err)
			return
		}
	...
	if err := writeM3U8File(content, m.recordPlayListFilename, m.recordPlayListFilenameBak); err != nil {
		m.Log().Error("[%s] write record m3u8 file error. err=%+v", m.UniqueKey, err)
	}
}
...
func (m *Muxer) writePlaylist(isLast bool) {
	...
	if err := writeM3U8File(buf.Bytes(), m.playlistFilename, m.playlistFilenameBak); err != nil {
		m.Log().Error("[%s] write live m3u8 file error. err=%+v", m.UniqueKey, err)
	}
}
